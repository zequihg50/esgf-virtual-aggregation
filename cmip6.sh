...
curl -s "http://esgf-data.dkrz.de/${search_facets}" | \
  jq -r ".facet_counts.facet_fields.${subfield}|map(strings)[]" > variables

parallel --gnu esgf-utils/esgf-search -i {2} -q "project=CMIP6 type=File variable_id={1} distrib=False" :::: variables :::: esgf-utils/indexnodes | jq -c '
